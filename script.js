...
function renderBlogArchive() {
    ...
    for (const [yearMonth, posts] of Object.entries(archive)) {
        html += `<h3>${new Date(yearMonth).toLocaleString('en-US', { year: 'numeric', month: 'long' })}</h3>`;
        posts.forEach(post => {
            html += `
                <p><a href="blog-post.html?id=${post.id}">${post.title}</a> - ${new Date(post.date).toLocaleDateString()}</p>
            `;
        });
    }
...
function renderBlogPost() {
    ...
    postContainer.innerHTML = `
        ...
        <p class="blog-date">${new Date(post.date).toLocaleDateString()}</p>
        <p>${post.content}</p>
        <a href="blog.html" class="cta-button secondary">Back to Blog</a>
    `;
}

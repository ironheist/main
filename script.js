...
const blogPosts = [
    {
        ...
        content: "Last night, I sat by the ocean, its waves a symphony of secrets. Each crash against the shore felt like a heartbeat, urging me to write. I’ve been working on a new poem for an upcoming collection, one that captures the vastness of longing—how it stretches like the horizon, endless yet unattainable. The ocean reminds me of *You and the Moon*, where distance is both a barrier and a muse. I wrote a line under the stars: *‘Your absence is a tide, pulling me under.’* What is it about the sea that makes us feel so small, yet so alive?",
        date: "2025-04-14",
    ...
    },
    {
        ...
        content: "This week, I opened *Garden of Grief* again, not to read but to remember. Each poem in that collection is a scar, healed but never forgotten. I found myself writing a new piece, something about how grief lingers like a guest who overstays. It’s strange how pain can be a muse, how it carves space for hope. I thought of a line from *A Letdown*: *‘You were my summer, my autumn, but now my winter.’* Writing feels like excavating my soul—painful, but necessary. Has a memory ever inspired you to create?",
        date: "2025-04-21",
...
function renderBlogList() {
    ...
    blogListContainer.innerHTML = blogPosts.sort((a, b) => new Date(b.date) - new Date(a.date)).map(post => `
        <div class="blog-post">
            <h3>${post.title}</h3>
            <p class="blog-date">${new Date(post.date).toLocaleDateString()}</p>
            <p>${post.excerpt}</p>
...
function renderBlogPost() {
    ...
    if (post) {
        document.title = `${post.title} | Iron Heist's Poetry`;
        ...
        postContainer.innerHTML = `<p>Post not found.</p><a href="blog.html" class="back-link">Back to Blog</a>`;
    }
}
...
function renderBlogArchive() {
    ...
    const postsByYearMonth = blogPosts.reduce((acc, post) => {
        ...
        if (!acc[year][key]) acc[year][key] = [];
        acc[year][key].push(post);
        return acc;
    }, {});

    let html = '';
    Object.keys(postsByYearMonth).sort((a, b) => b - a).forEach(year => {
        html += `<div class="archive-year">
                    <button class="archive-year-toggle">${year} <i class="fas fa-chevron-down"></i></button>
                    <div class="archive-months" style="display: none;">`;
        Object.keys(postsByYearMonth[year]).sort((a, b) => {
            const dateA = new Date(a);
            const dateB = new Date(b);
            return dateB - dateA;
        }).forEach(monthYear => {
            const posts = postsByYearMonth[year][monthYear];
            html += `<div class="archive-month">
                        <button class="archive-month-toggle">${monthYear}</button>
                        <ul class="archive-posts" style="display: none;">`;
            posts.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(post => {
                html += `<li><a href="blog-post.html?id=${post.id}">${post.title}</a></li>`;
            });
            html += `</ul></div>`;
        });
        html += `</div></div>`;
    ...
    // Add toggle functionality for years
    document.querySelectorAll('.archive-year-toggle').forEach(button => {
    ...
    document.querySelectorAll('.archive-month-toggle').forEach(button => {
        button.addEventListener('click', () => {
...
function searchBlogPosts() {
    ...
    const posts = blogPosts.filter(post => {
        ...
        const keywords = post.keywords.join(' ').toUpperCase();
        return title.includes(filter) || keywords.includes(filter);
    });

    if (posts.length === 0) {
        blogListContainer.innerHTML = '<p style="text-align: center; color: #888;">No posts found.</p>';
        setTimeout(renderBlogList, 2000);
    } else {
        blogListContainer.innerHTML = posts.sort((a, b) => new Date(b.date) - new Date(a.date)).map(post => `
            <div class="blog-post">
                <h3>${post.title}</h3>
                <p class="blog-date">${new Date(post.date).toLocaleDateString()}</p>
                <p>${post.excerpt}</p>
...
function redirectToRandomPoem() {
    ...
    window.open(`viewer src/viewer.html?pdf=${encodeURIComponent(randomBook.href)}&title=${encodeURIComponent(randomBook.title)}`, '_blank');
}
